    ...
    <div class="card-body">
        <form action="{{ route('admin.locations.store') }}" method="POST">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="map_id" class="form-label">Map ID</label>
                        <input type="text" class="form-control @error('map_id') is-invalid @enderror" 
                               id="map_id" name="map_id" value="{{ old('map_id') }}" 
                               placeholder="Click on map to select region" readonly>
                        <small class="text-muted">Click on the map below to select a region</small>
                        @error('map_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-3">
                    <div class="mb-3">
                        ...
                        <input type="color" class="form-control form-control-color @error('color') is-invalid @enderror" 
                               id="color" name="color" value="{{ old('color', '#1CEAB9') }}" style="height: 38px;">
                        @error('color')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="mb-4">
                <label class="form-label">Select Map Region <span class="text-muted">(click on a region)</span></label>
                <div class="map-selector-container">
                    <div class="selected-region-info" id="selectedRegionInfo">
                        <i class="bi bi-info-circle"></i> No region selected
                    </div>
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-check-lg"></i> Create Location
                </button>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        regions.forEach(region => {
            region.addEventListener('click', function() {
                ...
                selectedInfo.innerHTML = `<i class="bi bi-check-circle-fill"></i> Selected: <strong>${regionName}</strong> (${regionId})`;
                selectedInfo.classList.add('selected');

                // Update visual selection on map
                regions.forEach(r => r.classList.remove('selected'));
                // Select all regions with the same ID (for regions split across multiple groups)
