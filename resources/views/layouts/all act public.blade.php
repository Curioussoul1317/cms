        ...
        <ul class="navbar-nav">
            ...
            @foreach($mainCategories->where('is_published', 1) as $mainCategory)
                @if($mainCategory->id != 1)
                    <li class="nav-item" data-dropdown="main-category-{{ $mainCategory->id }}">
                        <span class="nav-link {{ 
            ...
            @foreach($corpCategories  as $corpCategory)
                <li class="nav-item" data-dropdown="corp-category-{{ $corpCategory->id }}">
                    <span class="nav-link {{ 
                        (isset($item) && isset($item->corpCategory) && $item->corpCategory->id == $corpCategory->id) ? 'active' : '' 
                    }}">
                        {{ $corpCategory->name }}
                        <i class="ti ti-chevron-down"></i>
                    </span>
...
{{-- Regular Category Dropdowns --}}
@foreach($mainCategories->where('is_published', 1) as $mainCategory)
    <div class="nav-dropdown-menu" data-dropdown-content="main-category-{{ $mainCategory->id }}">
        <div class="dropdown-container">
            <div class="dropdown-main">
                    ...
                    @if($page->id != 1)
                        <div class="category-section">
                            <div class="category-header">
                                ...
                                @else
                                    <a href="{{ route('content.show', ['page', $page->id]) }}" class="subcategory-link">
                                        <div class="category-icon">
                                            <i class="{{ $page->icon ?? 'ti ti-folder' }}"></i>
                                        </div>
                                        <div class="category-info">
                ...
                @empty
                    <div class="category-section">
                        <div class="text-muted text-center py-4">No pages available</div>
                    </div>
                @endforelse
            </div>
...
{{-- Corporate Category Dropdowns --}}
@foreach($corpCategories  as $corpCategory)
    <div class="nav-dropdown-menu" data-dropdown-content="corp-category-{{ $corpCategory->id }}">
        <div class="dropdown-container">
            <div class="dropdown-main">
                @forelse($corpCategory->corpPages->where('is_published', 1)->whereNull('parent_id') as $corpPage)
                    <div class="category-section">
                        <div class="category-header">
                            @if($corpPage->has_children && $corpPage->children->where('is_published', 1)->count() > 0)
                                <div class="category-icon">
                                    <i class="{{ $corpPage->icon ?? 'ti ti-folder' }}"></i>
                                </div>
                                <div class="category-info">
                                    <h3>{{ $corpPage->name }}</h3>
                                    @if($corpPage->description)
                                        <p>{{ Str::limit($corpPage->description, 100) }}</p>
                                    @endif
                                </div>
                            @else
                        ...
                        @if($corpPage->has_children && $corpPage->children->where('is_published', 1)->count() > 0)
                            <div class="subcategory-links">
                                @foreach($corpPage->children->where('is_published', 1)->sortBy('order') as $childCorpPage)
                                    <a href=" " 
                                       class="subcategory-link {{ (isset($item) && $item->id == $childCorpPage->id) ? 'active' : '' }}">
                                        <i class="ti ti-circle-filled"></i>
                                        {{ $childCorpPage->name }}
                                    </a>
                                @endforeach
                ...
                @empty
                    <div class="category-section">
                        <div class="text-muted text-center py-4">No corporate pages available</div>
                    </div>
                @endforelse
            </div>

            <div class="dropdown-sidebar">
                <h4 class="sidebar-title">CORPORATE SERVICES</h4>
                <div class="sidebar-links">
                    <a href="#" class="sidebar-link">
                        <i class="ti ti-circle-filled"></i>
...
<div class="mobile-nav-overlay">
    <div class="mobile-nav-content">
        ...
        <div class="mobile-categories-wrapper">
            ...
            {{-- Corporate Categories Mobile --}}
            @foreach($corpCategories->where('is_published', 1) as $corpCategory)
                <div class="mobile-category-section">
                    <div class="mobile-category-header">
                        {{ $corpCategory->name }}
                    </div>

                    @foreach($corpCategory->corpPages->where('is_published', 1)->whereNull('parent_id')->sortBy('order') as $corpPage)
                        @if($corpPage->has_children && $corpPage->children->where('is_published', 1)->count() > 0)
                            <div class="mobile-subcategory">
                                <div class="mobile-subcategory-header">
                                    <span>{{ $corpPage->name }}</span>
                                    <i class="ti ti-chevron-down"></i>
                                </div>
                                <div class="mobile-subcategory-links">
                                    @foreach($corpPage->children->where('is_published', 1)->sortBy('order') as $childCorpPage)
                                        <a href="{{ route('corp.content.show', ['corpPage', $childCorpPage->id]) }}" class="mobile-subcategory-link">
                                            {{ $childCorpPage->name }}
                                        </a>
                                    @endforeach
                                </div>
                        ...
                        @else
                            <a href="{{ route('corp.content.show', ['corpPage', $corpPage->id]) }}" class="mobile-simple-link">
                                {{ $corpPage->name }}
                            </a>
                        @endif
                    @endforeach
