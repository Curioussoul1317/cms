    ...
    <div class="container-xl">
        <div class="row g-2 align-items-center">
            <div class="col">
                ...
                <h2 class="page-title">
                    Content Blocks of @if($type=="sub-category") 
                        {{ $model->name }} 
                    @else 
                        {{ $model->title }} 
                    @endif 
            ...
            <div class="col-auto ms-auto d-print-none">
                <div class="btn-list">
                    <a href="{{ route('link-contents.create', ['type' => $type, 'id' => $model->id]) }}" 
                        class="btn btn-primary d-none d-sm-inline-block"> 
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M12 5l0 14"></path><path d="M5 12l14 0"></path></svg>                       
                        Add Content
                    </a>

                    <form action="{{ route('linkContent.approveModel', ['type' => $type, 'id' => $model->id]) }}" 
                          method="POST" 
                    ...
                    <form action="{{ route('linkContent.unapproveModel', ['type' => $type, 'id' => $model->id]) }}" 
                          method="POST" 
                          style="display: inline;">
                        @csrf
                        <button type="submit" 
                    ...
                    @if($type=="sub-category")
                        <a href="{{ route('content.show', ['subcategory', $model->id]) }}" 
                            class="btn btn-icon btn-cyan d-none d-sm-inline-block"
                    ...
                    @else
                        <a href="{{ route('content.show', ['link', $model->id]) }} " 
                                    ...
                                    <div class="col-auto ms-auto d-print-none">
                                        <div class="btn-list">
                                            @if(!$content->is_approved)
                                                <form action="{{ route('linkContent.toggleApproval', ['type' => $type, 'id' => $model->id, 'linkContent' => $content->id]) }}" 
                                                      method="POST" 
                                                      style="display: inline;">
                                                    @csrf
                                                    <button type="submit" 
                                            ...
                                            <a href="{{ route('link-contents.edit', ['type' => $type, 'id' => $model->id, 'linkContent' => $content]) }}" 
                                               class="btn btn-icon btn-yellow">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 7h-1a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-1" /><path d="M20.385 6.585a2.1 2.1 0 0 0 -2.97 -2.97l-8.415 8.385v3h3l8.385 -8.415z" /><path d="M16 5l3 3" /></svg>
                                            </a>
                                            <form action="{{ route('link-contents.destroy', ['type' => $type, 'id' => $model->id, 'linkContent' => $content]) }}" 
                                                  method="POST" 
                                                  class="d-inline"
        ...
        if (container) {
            const sortable = new Sortable(container, {
                ...
                onEnd: function(evt) {
                    ...
                    fetch('{{ route('link-contents.update-order', ['type' => $type, 'id' => $model->id]) }}', {
                        method: 'POST',
                        headers: {
