...
<head>
    ...
    * {
    ...
    .page-wrapper {
        flex: 1;
        padding-top: 23px; /* Offset for fixed navbar */
        background-color: white;
    }
            ...
            <div class="nav-dropdown-menu" data-dropdown-content="category-{{ $mainCategory->id }}">
                <div class="dropdown-container">
                    <div class="dropdown-main">
                        @forelse($mainCategory->subCategories->where('is_approved', 0) as $subCategory)
                    ...
                    @foreach($mainCategories->where('is_active', 1) as $mainCategory)
                        <div class="col-lg-2 col-md-6">
                            ...
                            <ul class="footer-links">
                                @foreach($mainCategory->subCategories->where('is_approved', 0)->sortBy('order') as $subCategory)
                                    @if($subCategory->subtype == 'SubCategory')
                                        <!-- SubCategory Type - Cyan colored label -->
                                        <li class="footer-section-label" style="color: rgb(29, 200, 226); margin-top: {{ $loop->first ? '0' : '1rem' }};">
                                            {{ $subCategory->name }}
                                        </li>
                                    @else
                                        <!-- Regular subcategory with link -->
                                        <li>
                                            <a href="{{ route('subcategory.show', $subCategory->id) }}">
                                                {{ $subCategory->name }}
                                            </a>
                                    ...
                                    <!-- Show links under this subcategory -->
                                    @foreach($subCategory->links->where('is_approved', 0)->sortBy('order') as $link)
                                        <li>
                                            <a href="{{ route('link.show', $link->id) }}">
                                                {{ $link->title }}
                                            </a>
